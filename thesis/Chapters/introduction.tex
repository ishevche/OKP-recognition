...
The most natural way to impose a limit on the number of crossings is to place an upper bound on the number of crossings per edge for some value \(k\). This class of graphs is among those that have attracted the most interest from the research community lately~\cite{contest}.\todo{How do I explain why We have done this step? (Thesis 4)} In this work, however, we went a step further, additionally restricting the vertices to lie on a circle. Drawings that comply with both restrictions are called \emph{outer \(k\)-planar}, and so are the graphs that admit such drawings.
