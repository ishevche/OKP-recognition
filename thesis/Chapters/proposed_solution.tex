...
As discussed in~\Cref{ch:related-work}, the studies conducted in the field of beyond planar graphs lie purely in the theoretical domain, lacking practical validation. In this thesis, we aim to address this gap by providing
\begin{itemize}
...
To transform the returned drawing into a picture, we use the \textsf{neato} layout engine~\cite{neato}. An example of the drawing returned by an ILP-based algorithm for \todo{Is it a good example? Or maybe ther should be multiple of them?}the Petersen graph is demonstrated in~\Cref{fig:demo}. Here, we use blue for edges that cross exactly \(k\) other edges, where \(k\) is the local circular crossing number.\todo{Should I mention that edges that exceed \(k\) crossings are coloured in red, but this should never happen?}
