...
Planar graphs have been thoroughly studied for decades as the simplest yet most interpretable class. The notion of planarity, however, imposes too harsh restrictions on the graph structure, significantly limiting their number. To resolve this issue, the research started shifting towards almost planar graphs. One such class is outer \(k\)-planar graphs. Unfortunately, the discussion about their recognition is held in a purely theoretical context, lacking practical validation. In this work, we focus on the practical aspect of the problem by implementing three different algorithms for recognising outer \(k\)-planar graphs. Two of these algorithms involve encoding the problem into other thoroughly studied NP-hard problems: integer linear programming and satisfiability. The last one is an implementation of a recently introduced algorithm based on a dynamic programming approach. Comparing the resulting implementation, we found out that one, based on the integer linear programming encoding, works best for most graphs due to the efficiency of the existing solvers and the simplicity of the encoding. The algorithm based on the satisfiability problem is more complex, requiring an exponential number of clauses in terms of the parameters of the input graph. The algorithm based on dynamic programming is outperformed by two others. However, it can present insights into how subgraphs can be drawn. In the future, these implementations can be used to solve the recognition problem for practical use and test any new implementations.
\end{abstract}
