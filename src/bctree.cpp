...
bctree_t decompose(const Graph& graph) {
    ...
    std::vector<size_t> edge_component(num_edges(graph));
    std::vector<Vertex> articulation_points;
    auto [num_components, _] = biconnected_components(
        graph,
    ...
    for (auto [ei, ei_end] = edges(graph); ei != ei_end; ++ei) {
        size_t component_id = edge_component[get(edge_index_map, *ei)];
        ...
        add_edge(vertex_mapping[source], vertex_mapping[target], num_edges(component), component);
    }
    ...
    for (size_t component_id = 0; component_id < num_components; ++component_id) {
        ...
        for (const auto& [global_v, local_v] : vertex_mappings[component_id]) {
            size_t local_v_index = get(vertex_index_map, local_v);
